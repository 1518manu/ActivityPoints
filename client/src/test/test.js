...
export const Student = ({ token, userData: initialUserData, onLogout }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        console.log("Fetching user data... email:", initialUserData);
  ...
  return (
    <div className="container">
      ...
      <div className="upload-div">
        <button className="upload-button" onClick={openUploadPage}>
          Upload <FaUpload style={{ color: "#fff", fontSize: "20px", margin: "5px 0px", fontWeight: "100" }} />
        </button>
      </div>
    </div>
  );
